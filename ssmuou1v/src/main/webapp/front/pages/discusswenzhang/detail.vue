...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusswenzhang');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusswenzhang');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusswenzhang', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscusswenzhang', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusswenzhang/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusswenzhang', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusswenzhang', this.detail);
				this.$utils.msg('审核成功');
